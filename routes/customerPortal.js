...
router.post('/login', async (req, res) => {
  try {
    ...
    // Check if login using customer_id (6 digit) or phone
    if (customer_id && /^\d{6}$/.test(customer_id)) {
      // Login dengan customer_id (6 digit)
      try {
        const billingManager = require('../config/billing');
        customer = await billingManager.getCustomerByCustomerId(customer_id);
        if (!customer) {
        ...
        if (req.xhr || req.headers.accept.indexOf('json') > -1) {
          ...
          return res.render('login', { settings, error: 'Terjadi kesalahan saat login.' });
        }
      }
    } else if (phone) {
      // Login dengan phone (existing logic)
      // Fast validation: terima 08..., 62..., +62...
      const valid = !!phone && (/^08[0-9]{8,13}$/.test(phone) || /^\+?62[0-9]{8,13}$/.test(phone));
      if (!valid) {
        if (req.xhr || req.headers.accept.indexOf('json') > -1) {
          return res.status(400).json({ success: false, message: 'Nomor HP harus valid (08..., 62..., atau +62...) atau gunakan ID Pelanggan 6 digit' });
        } else {
          return res.render('login', { settings, error: 'Nomor HP tidak valid atau gunakan ID Pelanggan 6 digit.' });
        }
      }
      ...
      try {
        const billingManager = require('../config/billing');
        customer = await billingManager.getCustomerByPhone(normalizedPhone);
      } catch (error) {
        console.error('Error getting customer by phone:', error);
      }
    } else {
      if (req.xhr || req.headers.accept.indexOf('json') > -1) {
        return res.status(400).json({ success: false, message: 'Masukkan Nomor HP atau ID Pelanggan 6 digit' });
      } else {
        return res.render('login', { settings, error: 'Masukkan Nomor HP atau ID Pelanggan 6 digit.' });
      }
    }
    ...
    if (settings.customerPortalOtp === true || String(settings.customerPortalOtp).toLowerCase() === 'true') {
      ...
      if (customer) {
        ...
        console.log(`✅ [LOGIN] Set session customer_username: ${customer.username}, customer_id: ${customer.customer_id} for phone: ${normalizedPhone}`);
      } else {
