...
*${getCompanyHeader()}*`,
    ...
    async sendPaymentReceivedNotification(paymentId) {
        try {
            ...
            // Generate dan kirim invoice PDF
            let pdfPath = null;
            try {
                ...
                const provider = this.getProvider();
                if (provider) {
                    const formattedNumber = this.formatPhoneNumber(customer.phone);
                    ...
                    if (result.success) {
                        logger.info(`✅ Payment notification with PDF sent to ${customer.phone}`);
                        this.incrementDailyMessageCount();
